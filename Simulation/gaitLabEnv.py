...
class MultiDOFLeg:
    '''
    Simulates a simplified biomechanical leg with 3 degrees of freedom (hip, knee, ankle),
    used to model joint motion based on applied torques. Provides a basic physics-based
    approximation of limb dynamics to enrich the gait environment with realistic motion variation.
    '''
    ...
    def reset(self):
        '''
        Initializes the leg state with small random joint angles and zero velocities.
        '''
        self.state = np.array([
    ...
    def step(self, torques):
        '''
        Updates joint positions and velocities based on input torques using a simple integration method.
        '''
        hip_torque, knee_torque, ankle_torque = torques
        I = 1.0  # Moment of inertia (simplified)
...
class GaitEnv(gym.Env):
    '''
    Custom Gymnasium environment that simulates a gait laboratory for training reinforcement learning agents.
    Integrates patient data and optionally a dynamic leg model (DOF) to support personalized rehabilitation recommendations.
    '''
    def __init__(self, data_file="Data/gait_lab_dataset.csv", train=True, test_size=0.2, use_dof_model=False):
        ...
        # Define observation space (with or without leg model)
    ...
    def reset(self, seed=None, options=None):
        '''
        Selects a new random patient and optionally resets the leg model state.
        Returns the full observation (patient + leg state if used).
        '''
    ...
    def step(self, action):
        '''
        Takes a step in the environment: evaluates the selected action against the ground truth (if available),
        applies DOF simulation if enabled, and returns observation, reward, and info.
        '''
    ...
    def evaluate_agent(self, model):
        '''
        Evaluates a trained agent over the full dataset and displays accuracy and diagnostic plots.
        '''
        self.rewards = []
        self.correct_flags = []
...
if __name__ == "__main__":
    '''
    Loads a trained DQN model and simulates predictions on unseen synthetic patients.
    Outputs recommendations and saves them to a CSV file.
    '''
